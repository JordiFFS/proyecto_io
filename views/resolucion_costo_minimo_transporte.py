...
def crear_grafo_transporte(orígenes, destinos, asignacion, costos, nombres_origenes, nombres_destinos):
    """
    Crea un gráfico interactivo del transporte de Coca-Cola
    """
    fig = go.Figure()
...
def mostrar_resolucion_costo_minimo_transporte(costos, oferta, demanda, orígenes, destinos):
    """
    Muestra la resolución del método de Costo Mínimo paso a paso
    """
    ...
    st.info("""
    **Algoritmo:**
    1. Encuentra la celda con menor costo disponible
    2. Asigna el máximo posible sin exceder oferta/demanda
    3. Marca fila o columna como agotada si es necesario
    ...
    for i in range(len(orígenes)):
        for j in range(len(destinos)):
            if resultado['asignacion'][i][j] > 0:
                ...
                costo_acumulado += costo_total_asign
...
def ejemplo_costo_minimo_transporte():
    """Ejemplo de Costo Mínimo con datos de Coca-Cola"""
    st.subheader("📦 Ejemplo: Método de Costo Mínimo - Coca-Cola")
    st.write("""
